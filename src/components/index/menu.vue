<template>
  <div class="m-menu">
    <dl class="nav" @mouseleave="hideenPanel">
      <dt>全部分类</dt>
      <dd v-for="(item, index) in menuList" :key="item.food + '--'  + index" @mouseenter="showPanel(item)">
        <i :class="item.type" />
        {{ item.name }}
        <span class="arrow"></span>
      </dd>
    </dl>
    <div class="detail" v-if="typeShow" @mouseleave="hideenPanel" @mouseenter="clearTimeout">
      <template v-for="(type, ind) in typeShow.items" >
        <h4 :key="type + '--'  + ind">{{ type.title }}</h4>
        <template v-if="type.items.length !== 0">
          <span v-for="(v, i) in type.items" :key="v + '--' + i">{{ v }}</span>
        </template>
      </template>
    </div>
  </div>
      ...
      menuListCopy: [
        {
          ...
          children: [
            {
              title: '美食',
              children: ['代金券', '甜点饮品', '火锅自助餐','小吃快餐', '日韩料理', '西餐', '聚餐宴请', '烧烤烤肉','东北菜','川湘菜','江浙菜','香锅烤鱼','粤港菜','中式烧烤/烤串','西北菜','咖啡酒吧茶馆','云贵菜','东南亚菜','海鲜','素食','台湾/客家菜','创意菜','汤/粥/炖菜','蒙餐新疆菜','其他美食', '京菜鲁菜']
            }
          ]
        },
        {
        ...
        },
        {
          ...
          children: [
            {
              title: '休闲娱乐',
              children: ['足疗按摩', '洗浴/汗蒸', '酒吧', '密室逃脱','轰趴馆','茶馆','私人影院','DIY手工坊','采摘/农家乐','网吧网咖','游乐游艺','VR','桌面游戏','真人CS','棋牌室','其他玩乐']
            },
            {
              title: 'KTV',
              children: ['KTV']
        ...
        },
        {
          ...
          children: [
            {
              title: '丽人',
              children: ['美发','美甲美睫','医学美容','瑜伽舞蹈','瘦身纤体','韩式定妆','祛痘','纹身','化妆品','养发']
            }
          ]
        },
        {
        ...
        },
        {
          ...
          children: [
            ...
            {
              title: '装修建材',
              children: ['地板','瓷砖石材','橱柜','灯饰照明','厨卫洁具','油漆涂料','集成吊顶','墙纸铅艺','门窗','木材板材','家用五金','电工电料','楼梯','管材管件']
            },
            {
              title: '家具家居',
              children: ['家居','床上用品/家纺','家居饰品','厨具餐具','智能家具']
            },
            {
              title: '家装卖场',
        ...
        },
        {
          ...
          children: [
            ...
            {
              title: '职业技术',
              children: ['美容美妆','会计','IT','厨艺','管理培训','摄影培训','司法考试','公务员考试','其他职业培训']
            },
            {
              title: '外语培训',
